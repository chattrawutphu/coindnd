let items = [
    ...
    import _ from './core/lodash.min.js';

    // Array operations
    function moveItem(sourceId, targetId, side, items = items) {
        const findInAll = (id) => _.find(_.flatMapDeep(items, i => [i, i.children, i.conditions, i.actions]), {id});
        const source = findInAll(sourceId);
        const target = findInAll(targetId);
        if (!source || !target) return "Pass";
    
        const arrays = _.flatMapDeep(items, i => [i.children, i.conditions, i.actions]);
        const sourceArray = _.find(arrays, arr => _.includes(arr, source));
        const targetArray = _.find(arrays, arr => _.includes(arr, target));
    
        _.pull(sourceArray, source);
        targetArray.splice(_.indexOf(targetArray, target) + (side === "bottom" ? 1 : 0), 0, source);
    
        return "Pass";
    }
    
    function addItem(newItem, targetId, items = items) {
        const target = _.find(_.flatMapDeep(items, i => [i, i.children, i.conditions, i.actions]), {id: targetId});
    ...
    function removeItem(id, items = items) {
        const remove = (arr) => {
            ...
            return _.some(arr, item => item.children && remove(item.children));
        };
        remove(items);
        return "Pass";
    }
    
    function updateItem(id, updates, items = items) {
        const update = (arr) => {
            ...
            return _.some(arr, subItem => subItem.children && update(subItem.children));
        };
        update(items);
        return "Pass";
    }
